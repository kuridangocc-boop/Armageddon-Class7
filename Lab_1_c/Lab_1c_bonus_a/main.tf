...
resource "aws_vpc" "armageddon_lab" {
...
resource "aws_internet_gateway" "armageddon_lab_igw01" {
...
resource "aws_subnet" "armageddon_lab_public_subnets" {
...
resource "aws_subnet" "armageddon_lab_private_subnets" {
...
resource "aws_eip" "armageddon_lab_nat_eip01" {
  domain = "vpc"
...
resource "aws_nat_gateway" "armageddon_lab_nat01" {
  allocation_id = aws_eip.armageddon_lab_nat_eip01.id
  subnet_id     = aws_subnet.armageddon_lab_public_subnets[0].id # NAT in a public subnet

  tags = {
    Name = "${local.name_prefix}-nat01"
  }

  depends_on = [aws_internet_gateway.armageddon_lab_igw01]
}
...
resource "aws_route_table" "armageddon_lab_public_rt01" {
  vpc_id = aws_vpc.armageddon_lab.id
...
resource "aws_route" "armageddon_lab_public_default_route" {
  route_table_id         = aws_route_table.armageddon_lab_public_rt01.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_internet_gateway.armageddon_lab_igw01.id
}

# Explanation: Attach public subnets to the “public lanes.”
resource "aws_route_table_association" "armageddon_lab_public_rta" {
  count          = length(aws_subnet.armageddon_lab_public_subnets)
  subnet_id      = aws_subnet.armageddon_lab_public_subnets[count.index].id
  route_table_id = aws_route_table.armageddon_lab_public_rt01.id
}

# Explanation: Private route table = “stay hidden, but still ship supplies.”
resource "aws_route_table" "armageddon_lab_private_rt01" {
  vpc_id = aws_vpc.armageddon_lab.id
...
resource "aws_route" "armageddon_lab_private_default_route" {
  route_table_id         = aws_route_table.armageddon_lab_private_rt01.id
  destination_cidr_block = "0.0.0.0/0"
  nat_gateway_id         = aws_nat_gateway.armageddon_lab_nat01.id
}

# Explanation: Attach private subnets to the “stealth lanes.”
resource "aws_route_table_association" "armageddon_lab_private_rta" {
  count          = length(aws_subnet.armageddon_lab_private_subnets)
  subnet_id      = aws_subnet.armageddon_lab_private_subnets[count.index].id
  route_table_id = aws_route_table.armageddon_lab_private_rt01.id
}
...
# resource "aws_security_group" "armageddon_lab_ec2_sg01" {
#   name        = "${local.name_prefix}-ec2-sg01"
#   description = "EC2 app security group"
#   vpc_id      = aws_vpc.armageddon_lab_vpc01.id
...
resource "aws_security_group" "armageddon_lab_rds_sg01" {
  name        = "${local.name_prefix}-rds-sg01"
  description = "RDS security group"
  vpc_id      = aws_vpc.armageddon_lab.id
  ...
  ingress {
    ...
    to_port         = 3306
    protocol        = "tcp"
    security_groups = [aws_security_group.armageddon_lab_ec2_sg01.id]
  }
...
resource "aws_db_subnet_group" "armageddon_lab_rds_subnet_group01" {
...
resource "aws_db_instance" "armageddon_lab_rds01" {
  ...
  db_subnet_group_name   = aws_db_subnet_group.armageddon_lab_rds_subnet_group01.name
  vpc_security_group_ids = [aws_security_group.armageddon_lab_rds_sg01.id]
...
resource "aws_iam_role" "armageddon_lab_ec2_role01" {
...
resource "aws_iam_role_policy_attachment" "armageddon_lab_ec2_ssm_attach" {
...
resource "aws_iam_role_policy_attachment" "armageddon_lab_ec2_secrets_attach" {
  role       = aws_iam_role.armageddon_lab_ec2_role01.name
...
resource "aws_iam_role_policy_attachment" "armageddon_lab_ec2_cw_attach" {
  role       = aws_iam_role.armageddon_lab_ec2_role01.name
  policy_arn = "arn:aws:iam::aws:policy/CloudWatchAgentServerPolicy"
}

# Explanation: Instance profile is the harness that straps the role onto the EC2 like bandolier ammo.
resource "aws_iam_instance_profile" "armageddon_lab_instance_profile01" {
...
# resource "aws_instance" "armageddon_lab_ec201" {
#   ami                    = var.ec2_ami_id
#   instance_type          = var.ec2_instance_type
#   subnet_id              = aws_subnet.armageddon_lab_public_subnets[0].id
...
resource "aws_ssm_parameter" "armageddon_lab_db_endpoint_param" {
...
resource "aws_ssm_parameter" "armageddon_lab_db_port_param" {
  name  = "/lab/db/port"
  type  = "String"
  value = tostring(aws_db_instance.armageddon_lab_rds01.port)
...
resource "aws_ssm_parameter" "armageddon_lab_db_name_param" {
  name  = "/lab/db/name"
  type  = "String"
  value = var.db_name
...
resource "aws_secretsmanager_secret" "armageddon_lab_db_secret01" {
  name                    = "${local.name_prefix}/rds/mysql01"
...
# aws secretsmanager restore-secret --secret-id armageddon_lab/rds/mysql


# Explanation: Secret payload—students should align this structure with their app (and support rotation later).
resource "aws_secretsmanager_secret_version" "armageddon_lab_db_secret_version01" {
  secret_id = aws_secretsmanager_secret.armageddon_lab_db_secret01.id

  secret_string = jsonencode({
    username = var.db_username
    password = var.db_password
    host     = aws_db_instance.armageddon_lab_rds01.address
    port     = aws_db_instance.armageddon_lab_rds01.port
    dbname   = var.db_name
  })
}
...
resource "aws_cloudwatch_log_group" "armageddon_lab_log_group01" {
...
resource "aws_cloudwatch_metric_alarm" "armageddon_lab_db_alarm01" {
  ...
  alarm_actions = [aws_sns_topic.armageddon_lab_sns_topic01.arn]
...
resource "aws_sns_topic" "armageddon_lab_sns_topic01" {
...
resource "aws_sns_topic_subscription" "armageddon_lab_sns_sub01" {
  topic_arn = aws_sns_topic.armageddon_lab_sns_topic01.arn
  protocol  = "email"
